...
import { UploadActions, CheckApi, UploadApi, UploadActionProgress, MergeApi } from "./interface.ts";
import { AxiosResponse } from "axios";
import { FileTask } from "../task/FileTask";

export function getCheckChunkApi(action: UploadActions["check"], task: FileTask): CheckApi {
    ...
    const { action: url, method, timeout, transformPrams, transformData, strategy } = action;
    //normalize action
...
export function getUploadChunkApi<D = any>(action: UploadActions["upload"], task: FileTask): UploadApi<D> {
    const { action: url, method, file: fileField = "file", timeout, transformData, transformParams } = action;
...
export function getMergeChunkApi(action: UploadActions["merge"], task: FileTask): MergeApi {
    const { action: url, method, timeout, transformPrams, transformData } = action;
